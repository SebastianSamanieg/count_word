...
def search_documents_with_words(documents, inverted_index):
    matching_documents = {}

    for word, document_ids in inverted_index.items():
        matching_documents[word] = document_ids
...
inverted_index = create_inverted_index(my_documents)

# Obtener documentos para todas las palabras
all_words = inverted_index.keys()
matching_documents = search_documents_with_words(all_words, inverted_index)
...
# Imprimir los resultados y el tiempo de ejecución
print("Índice Invertido:")
for word, document_ids in matching_documents.items():
